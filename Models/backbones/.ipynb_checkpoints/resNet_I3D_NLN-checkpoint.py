...
class I3Res18(nn.Module):

    def __init__(self, block=Bottleneck, layers=[2, 2, 2, 2], use_nl=False, data_confs=None, model_confs=None):
        num_classes = data_confs.num_classes
        self.inplanes = 64
        super(I3Res18, self).__init__()
...
def resNet18_I3D(pretrained=False, dataset_name=None, **kwargs):
    model = I3Res18(use_nl=False, **kwargs)
    
    model_dict = model.state_dict()
    pretrained_dict = torch.load('weights/pretrained/i3d_r50_kinetics.pth')
...
def resNet18_I3D_NLN(pretrained=False, dataset_name=None, **kwargs):
    model = I3Res18(use_nl=True, **kwargs)
    model_dict = model.state_dict()
    pretrained_dict = torch.load('weights/pretrained/i3d_r50_nl_kinetics.pth')
    # 1. filter out unnecessary keys
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
    # 2. overwrite entries in the existing state dict
    model_dict.update(pretrained_dict)
