...
#         f_div_C = self.Pruning(f_div_C, K=(f_div_C.size(-1)//2))*f_div_C
    ...
    def Pruning(self, matrixs, K=6):
        # matrix: b,s,s
        values, indices = torch.topk(matrixs, K, largest=False)
        mask = torch.ones_like(matrixs)
        for i in range(matrixs.size(0)):
            for j in range(matrixs.size(1)):
                mask[i, j, indices[i, j]] = 0
        return mask
