...
class Mask(object):
	...
	def get_InteractionMap(self):
		InteractionMap = torch.zeros(self.T*self.S, self.T*self.S)
		for t in xrange(self.T):
			for s in xrange(self.S):
				mask = self.get_personal_mask(t, s)
				#print t
				#show_map(mask)
				self.set_map(t, s, InteractionMap, mask)
		return InteractionMap


	def set_map(self, t, s, InteractionMap, mask):
		assert mask is not None, 'mask is None!'
		assert mask.size() == (self.T, self.S), 'dim should be (num_players, num_frames)'
		for _t in xrange(mask.size(0)):
			for _s in xrange(mask.size(1)):
				if mask[_t][_s]:
					#print t,s,_t,_s
					InteractionMap[t*self.S+s][_t*self.S+_s] = 1
					#InteractionMap[_s*_t][s*t] = 1
...
if __name__ == '__main__':
	...
	InteractionMap = Mask('Spatial').get_InteractionMap()
	Map = InteractionMap.numpy().tolist()
	show_map(Map)
